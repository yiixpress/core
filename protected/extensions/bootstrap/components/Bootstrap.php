...
class Bootstrap extends CApplicationComponent
{
    ...
    /**
    * Load the css, js files in minified or full version
    * 
    * @var mixed
    */
    protected $_debugMode = false; // can set to YII_DEBUG
	...
	public function init()
	{
        ...
        // Any AJAX that need bootstrap's CSS and JS must manually load them
	...
	{
		$this->registerAssetCss('bootstrap' . (!$this->_debugMode ? '.min' : '') . '.css');
	}
	...
	public function registerResponsiveCss()
	{
		...
		$cs->registerCssFile($this->getAssetsUrl() . '/css/bootstrap-responsive' . (!$this->_debugMode ? '.min' : '') . '.css');
	}
	...
	public function registerJS($position = CClientScript::POS_HEAD)
	{
		...
		$cs->registerScriptFile($this->getAssetsUrl() . '/js/bootstrap' . (!$this->_debugMode ? '.min' : '') . '.js', $position);
	}
