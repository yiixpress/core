...
class CrudCode extends CCodeModel
{
	...
	public function prepare()
	{
        ...
        $this->files[]=new CCodeFile(
            $this->Module->BasePath.DIRECTORY_SEPARATOR.'services'.DIRECTORY_SEPARATOR.$this->getModelClass().'Api.php',
            $this->render($serviceTemplateFile)
        );

        // service map
		$serviceMapTemplateFile=$templatePath.DIRECTORY_SEPARATOR.'serviceMap.php';

		$this->files[]=new CCodeFile(
			$this->Module->BasePath.DIRECTORY_SEPARATOR.'services'.DIRECTORY_SEPARATOR.'maps'.DIRECTORY_SEPARATOR.$this->getModelClass().'ServiceMap.php',
			$this->render($serviceMapTemplateFile)
		);
	...
	{
		return "\$form->labelEx(\$model,'{$column->name}',".str_replace("\n","",var_export($htmlOptions,true)).")";
	}
    ...
    ///////////////////////   CUSTOM CODE FOR FORM BUILDER ////////////////////////
    
    /**
    * The form user created using form builder
    */
    public function getForm() {
         ...
         */
         $table=CActiveRecord::model($class)->tableSchema;
         Yii::import('Cms.models.Form');
         $form = Form::model()->findByAttributes(array('table_name' => $table->name));
         return $form;
    }
