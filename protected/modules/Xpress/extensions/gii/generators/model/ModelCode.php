...
class ModelCode extends CCodeModel
{
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
			...
			$params=array(
                ...
                'moduleId'=>str_replace('.models','',$this->modelPath),
            ...
            if (is_object($this->form))
            {
                ...
                $params['lookupColumns'] = $lookupColumns;
            }
            else
            {
                $params['form'] = '';
                $params['lookupColumns'] = '';
            }
            
            $file = Yii::getPathOfAlias($this->modelPath).'/base/'.$className.'Base.php';
            if ((file_exists($file) && !is_writeable($file)) || !is_writable(dirname($file)))
            {
                $this->addError('modelPath', "Can't write file: ".$file);
                return;
            }
            //ModelBase file
            ...
            $file = Yii::getPathOfAlias($this->modelPath).'/'.$className.'.php';
            if ((file_exists($file) && !is_writeable($file)) || !is_writable(dirname($file)))
            {
                $this->addError('modelPath', "Can't write file: ".$file);
                return;
	...
	public function generateRules($table)
	{
        ...
        // auto set creation_datetime and last_update values
        $gii = Yii::app()->getModule('gii');
        $creation_field = $gii->Params['model']['created_time_field_name'];
        $hasCreationField = false;
        $update_field = $gii->Params['model']['last_update_time_field_name'];
		...
		foreach($table->columns as $column)
		{
            ...
            if ($column->name == $creation_field) $hasCreationField = true;
            if ($column->name == $update_field) $hasUpdateField = true;
		}
		if($required!==array())
        ...
        if ($hasCreationField)
            $rules[] = "array('{$creation_field}','default','value'=>new CDbExpression('NOW()'),'setOnEmpty'=>false,'on'=>'insert')";
        if ($hasUpdateField)
            $rules[] = "array('{$update_field}','default','value'=>new CDbExpression('NOW()'),'setOnEmpty'=>false,'on'=>'update')";
