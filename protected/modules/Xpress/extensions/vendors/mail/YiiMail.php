...
class YiiMail extends CApplicationComponent
{
    ...
    public function init() {
        ...
        parent::init();
        // use mail server configuration from Settings
        if (hasParam('SETTINGS_MAIL_METHOD'))
            $this->transportType = SETTINGS_MAIL_METHOD;
        if ($this->transportType == 'smtp')
        {
            $host = hasParam('SETTINGS_SMTP_HOST');
            $username = hasParam('SETTINGS_SMTP_USERNAME');
            $password = hasParam('SETTINGS_SMTP_PASSWORD');
            $port = hasParam('SETTINGS_SMTP_PORT');
            $secure = hasParam('SETTINGS_SMTP_SECURE');
            if ($host !== false  && !empty($host) && $port !== false)
            {
            ...
            if (!empty($username))
                $this->transportOptions['username']=$username;
            if (!empty($password))
                $this->transportOptions['password']=$password;
            if (!empty($secure))
                $this->transportOptions['encryption']=$secure;
        }
    }
